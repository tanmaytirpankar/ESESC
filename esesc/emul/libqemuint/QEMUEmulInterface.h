...
///MnB:~
  void queueInstruction(AddrType pc, AddrType addr, FlowID fid, int op, int src1, int src2, int dest, int dest2, bool inEmuTiming, AddrType smode) {
    //if(smode) MSG("QEMUEmulInterface\t%lx\t%lx", addr, smode);
    reader->queueInstruction(pc,addr, fid, op, src1, src2, dest, dest2, inEmuTiming, smode);
  //void queueInstruction(AddrType pc, AddrType addr, FlowID fid, int op, int src1, int src2, int dest, int dest2, bool inEmuTiming, void *oldData, void *newData) {
    //reader->queueInstruction(pc,addr, fid, op, src1, src2, dest, dest2, inEmuTiming, oldData, newData);
///:.
  }
