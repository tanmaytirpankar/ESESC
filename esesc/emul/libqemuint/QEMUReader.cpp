...
///MnB:~
void QEMUReader::queueInstruction(AddrType pc, AddrType addr, FlowID fid, int op, int src1, int src2, int dest, int dest2, bool keepStats, AddrType smode)
//void QEMUReader::queueInstruction(AddrType pc, AddrType addr, FlowID fid, int op, int src1, int src2, int dest, int dest2, bool keepStats, void *oldData, void *newData)
///:.
/* queue instruction (called by QEMU) {{{1 */
{
  ...
  while (tsfifo[fid].full()) {
    //MSG("tsfifo[%d].realsize(): %d", fid, tsfifo[fid].realsize());
...
///MnB:~ Memory Data
  rinst->set(pc,addr,static_cast<InstOpcode>(op), static_cast<RegType>(src1),static_cast<RegType>(src2),static_cast<RegType>(dest), static_cast<RegType>(dest2), keepStats, smode);
  //rinst->set(pc,addr,static_cast<InstOpcode>(op), static_cast<RegType>(src1),static_cast<RegType>(src2),static_cast<RegType>(dest), static_cast<RegType>(dest2), keepStats, oldData, newData);
  //if(rinst->getSMode())
  //MSG("MnB: queueInstruction\t%lx\t%lx(%lx)", rinst->getAddr(), rinst->getSMode(), smode);
///:.
...
{
  printf("** QEMUReader::syscall **\n");
  return; ///MnB:~
  RAWDInst *rinst = tsfifo[fid].getTailRef();

///MnB:~ Memory Data
  rinst->set(0xdeaddead,0,iRALU,LREG_R0,LREG_R0,LREG_InvalidOutput, LREG_InvalidOutput,true,0);
  //rinst->set(0xdeaddead,0,iRALU,LREG_R0,LREG_R0,LREG_InvalidOutput, LREG_InvalidOutput,true, NULL, NULL);
///:.
...
bool QEMUReader::populate(FlowID fid) {
  ...
  for(int i=32;i<tsfifo[fid].size();i++) {
    ...
    *dinsth = DInst::create(rinst->getInst(), rinst->getPC(), rinst->getAddr(), fid, rinst->getStatsFlag());
    //MSG("MnB: QEMUReader::populate\t%lx\t%lx\n", rinst->getAddr(), rinst->getSMode());
    (*dinsth)->smode = rinst->getSMode();
...
DInst *QEMUReader::peekHead(FlowID fid) {
  ...
  do{
    //MSG("QEMUReader::peekHead(FlowID %d)", fid);
    if (!ruffer[fid].empty()) {
