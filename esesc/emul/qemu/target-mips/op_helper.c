...
//#define WRDBITS 2
//#define BLKBITS 6
//#define WRDSIZE (1<<WRDBITS)
//#define BLKSIZE (1<<BLKBITS//
//  //static uint64_t tmpCount;
//  //static uint64_t oldPc;
//  //static uint64_t oldTarget;
//  //static uint64_t oldOp;
//  //static unsigned char oldData[BLKSIZE];
//  //static bool sent = false;
//  //static bool valid = false;
//  //static int oldSrc1;
//  //static int oldSrc2;
//  //static int oldDest;
//  //static CPUMIPSState *oldEnv = NULL;
//  //static CPUState *oldCpu = NULL////
//  // Note: Store instruction does not send updated data to simu. Tweaked translate.c to get latest data. 
//  // Logic to handle pass of old data and new data//
//  // iLALU_LD(10), iSALU_LL(12)
//  if((op == 10) || (op == 12)) {
//    uint64_t *addr = (uint64_t *)((target >> BLKBITS) << BLKBITS);
//    unsigned char ptr[BLKSIZE];
//    memcpy(ptr, addr, BLKSIZE);
//    icount = QEMUReader_queue_inst(pc, target, cpu->fid, op, src1, src2, dest, env, ptr, NULL);
//  }
//  // iSALU_ST(11), and iSALU_SC(13)
//  else if((op == 11) || (op == 13)) {
//    //if(pc == oldPc) {
//      //tmpCount--;
//      uint64_t *waddr = (uint64_t *)((target >> WRDBITS) << WRDBITS);
//      uint64_t *daddr = (uint64_t *)((target >> BLKBITS) << BLKBITS);
//      unsigned char new[WRDSIZE] ;
//      unsigned char ptr[BLKSIZE];
//      memcpy(new, waddr, WRDSIZE);
//      memcpy(ptr, daddr, BLKSIZE);
//      icount = QEMUReader_queue_inst(pc, target, cpu->fid, op, src1, src2, dest, env, ptr, new);
//      //sent = true;
//      //printf("%lld\n", op);
//    //}
//    //else {
//      //tmpCount++;
//      //if(valid && !sent) {
//      //  icount = QEMUReader_queue_inst(oldPc, oldTarget, oldCpu->fid , oldOp, oldSrc1, oldSrc2, oldDest, oldEnv, oldData, NULL);
//      //  printf("%lld\t%llx\n", op, oldPc);
//      //}
//      //uint64_t *addr = (uint64_t *)((target >> BLKBITS) << BLKBITS);
//      //memcpy(oldData, addr, BLKSIZE);
//      //valid = true;
//      //sent = false;
//      //printf("%lld\t", tmpCount);
//    //}
...
//  }
//  else {
//    icount = QEMUReader_queue_inst(pc, target, cpu->fid, op, src1, src2, dest, env, NULL, NULL);
//  }
...
///MnB:~ Memory Data
  ...
  //icount = QEMUReader_queue_inst(pc, 0, cpu->fid, op, src1, src2, dest, env, NULL, NULL);
///:.
}
#endif
