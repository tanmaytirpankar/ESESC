/* Multitable Bingo */
/* PC+Address > PC+Offset > Address > PC > Offset */
...
class PatternHistoryTable : LRUSetAssociativeCache<PatternHistoryTableData> {
  ...
  public:
    PatternHistoryTable(
        ...
        : Super(size, num_ways), pattern_len(pattern_len), addr_width(addr_width),
          pc_width(pc_width) {
...
class Bingo {
  public:
    Bingo(int pattern_len, int addr_width, int pc_width, int pattern_history_table_size,
        ...
        : pattern_len(pattern_len), filter_table(filter_table_size),
          ...
          pht_pc_address(pattern_history_table_size, pattern_len, addr_width, pc_width),
          pht_pc_offset(pattern_history_table_size, pattern_len, __builtin_ctz(pattern_len), pc_width),
          pht_address(pattern_history_table_size, pattern_len, addr_width, 0),
          pht_pc(pattern_history_table_size, pattern_len, 0, pc_width),
          pht_offset(pattern_history_table_size, pattern_len, __builtin_ctz(pattern_len), 0) {}
    ...
    vector<uint64_t> access(uint64_t block_number, uint64_t pc) {
        ...
        if (!entry) {
            ...
            vector<bool> pattern = this->find_in_pht(pc, block_number);
            if (pattern.empty())
                return vector<uint64_t>();
  ...
  private:
    vector<bool> find_in_pht(uint64_t pc, uint64_t address) {
        if (this->debug_level >= 1) {
            cerr << "[Bingo] find_in_pht(pc=" << pc << ", address=" << address << ")" << endl;
        }

        const int N = 5;
        vector<bool> pattern[N];
        pattern[0] = this->pht_pc_address.find(pc, address);
        pattern[1] = this->pht_pc_offset.find(pc, address);
        pattern[2] = this->pht_address.find(pc, address);
        pattern[3] = this->pht_pc.find(pc, address);
        pattern[4] = this->pht_offset.find(pc, address);
    ...
    void insert_in_pht(const AccumulationTable::Entry &entry) {
        ...
        this->pht_pc_address.insert(pc, address, pattern);
        this->pht_pc_offset.insert(pc, address, pattern);
        this->pht_address.insert(pc, address, pattern);
        this->pht_pc.insert(pc, address, pattern);
        this->pht_offset.insert(pc, address, pattern);
    }
...
void dram_prefetcher_initialize_(uint32_t cpu) {
    ...
    prefetchers = vector<Bingo>(NUM_CPUS, Bingo(REGION_SIZE >> LOG2_BLOCK_SIZE, ADDR_WIDTH,
